...
import Judge_pn_table # ツイートのネガポジ度を判断
...
class Window(QWidget):
    ...
    def search_button_event(self, s, ps, ns):
        ...
        negapoji_list = Judge_pn_table.Judge(tweet_list)
        for i in range(len(tweet_list)):
            text_and_score.setdefault(tweet_list[i], negapoji_list[i])
        ...
        for k,v in sorted(text_and_score.items(), key=lambda x:x[1], reverse=True):
            if v >= Judge_pn_table.divide_line:
